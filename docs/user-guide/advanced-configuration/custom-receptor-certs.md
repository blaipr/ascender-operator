...
```bash
kubectl create secret tls ascender-demo-receptor-ca \
   --cert=/path/to/ca.crt --key=/path/to/ca.key
```

The secret should be named `{AWX Custom Resource name}-receptor-ca`. In the above the AWX CR name is "ascender-demo". Please replace "ascender-demo" with your AWX Custom Resource name.

If this secret is created after Ascender is deployed, run the following to restart the deployment,

```bash
kubectl rollout restart deployment ascender-demo
```
